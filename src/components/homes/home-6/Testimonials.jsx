import { reviews as staticReviews, slidesData as staticSlidesData } from "@/data/testimonials";
import { Autoplay } from "swiper/modules";
import { Swiper, SwiperSlide } from "swiper/react";
import { useTestimonials } from "@/api/content";

export default function Testimonials({ useBackend = true }) {
  // Fetch data from backend if useBackend is true
  const { testimonials: backendTestimonials, loading } = useTestimonials();

  // Transform backend data to match expected format for slidesData
  const slidesData = useBackend && backendTestimonials.length > 0
  ...
  // Transform backend data to match expected format for reviews
  const reviews = useBackend && backendTestimonials.length > 0
    ? backendTestimonials.map(t => ({
        name: t.name,
  ...
  return (
    ...
    >
      <div className="section-outer panel py-6 lg:py-8 xl:py-6 bg-secondary text-black rounded-2 lg:rounded-3 m-2">
              ...
              >
                {slidesData.map((slide, index) => (
                  <SwiperSlide className="swiper-slide" key={index}>
                    <div className="panel vstack justify-between gap-3 p-3 lg:p-4 xl:p-6 bg-white text-black rounded-1-5" style={{height: "320px"}}>
