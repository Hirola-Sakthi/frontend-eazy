import { integrations as staticIntegrations } from "@/data/integrations";
import React from "react";
import { Link } from "react-router-dom";
import { useIntegrations } from "@/api/content";

export default function Integrations({ useBackend = true }) {
  // Fetch data from backend if useBackend is true
  const { integrations: backendIntegrations, loading } = useIntegrations();

  // Use backend data if available, otherwise fallback to static data
  const integrations = useBackend && backendIntegrations.length > 0
    ? backendIntegrations.map(i => ({
        name: i.name,
        category: i.category,
